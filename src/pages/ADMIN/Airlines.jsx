...
import { clearDeletedDataStatus, getAllAirlines } from '../../services/slices/ADMIN/AirlineSlice'
import { useState } from 'react'
import AirlineList from '../../components/ADMIN/core/airline/AirlineList'
...
const Airlines = () => {
    ...
    const { airline_data, deleted_airline_data, loading } = useSelector(state => state.airlineSlice);

    const AIRLINE_DATA = airline_data?.data?.filter(item => item?.isDeleted === false);
    const D_AIRLINE_STATUS = deleted_airline_data?.success;
    ...
    const newData = AIRLINE_DATA?.slice(pagesVisited, pagesVisited + userPerPage);
    const pageCount = Math.ceil((AIRLINE_DATA?.length || 0) / userPerPage);
    ...
    useEffect(() => {
        ...
        return () => {
            dispatch(clearDeletedDataStatus());
        }
    }, [dispatch, D_AIRLINE_STATUS]);
                                ...
                                <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                                    ...
                                    <div className="page-title-right">
                                        <ol className="breadcrumb m-0">
                                            ...
                                            <li className="breadcrumb-item active">Airlines</li>
                                        </ol>
                                    </div>
                                ...
                                <div className="card mb-0">
                                    <div className="card-body">
                                        <div className="ml-auto float-end">
                                            <Link to="/add-airline"
                                                className="btn btn-primary btn-sm btn-rounded waves-effect waves-light"><i
                                                    className="bx bx-plus me-1"></i> Add Airline</Link>
                                        </div>
                                        ...
                                        <table className="table">
                                            ...
                                            <tbody>
                                                {
                                                    AIRLINE_DATA?.length > 0 ?
                                                        AIRLINE_DATA?.filter((item) => {
                                                            if (search === "") {
                                                                return item;
                                                            }
                                                            return (
