...
import { editVendor } from '../../../services/slices/ADMIN/VendorSlice';
...
const EditVendor = () => {
    ...
    const ALL_VENDOR_DATA = JSON.parse(window.localStorage.getItem("all_vendor_data"));
    const filteredData = ALL_VENDOR_DATA?.filter(item => item?._id === vendor_id);

    const [formData, setFormData] = useState({
        vendor_name: filteredData[0].vendor_name,
        reporting_person_name: filteredData[0].reporting_person_name,
        reporting_person_email: filteredData[0].reporting_person_email,
        reporting_person_phone: filteredData[0].reporting_person_phone,
        reporting_person_alt_phone: filteredData[0].reporting_person_alt_phone,
        HO_address: filteredData[0].HO_address,
        status: filteredData[0].status,
        _airlineId: user?.id
    });
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { loading } = useSelector(state => state.vendorAirlineSlice);
    ...
    const handleSubmit = (e) => {
        ...
        const data = { ...formData };
        // console.log(data);
        dispatch(editVendor({ vendor_id, data, navigate }));
    }
                                ...
                                <div className="card mb-0">
                                    ...
                                    <form onSubmit={handleSubmit}>
                                        ...
                                        <div className="card-footer">
                                            <button className="btn btn-sm btn-primary">Update Vendor</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
