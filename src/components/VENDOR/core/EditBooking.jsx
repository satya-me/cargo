...
import { updateBookingData } from '../../../services/slices/VENDOR/VendorCargoBookingSlice';
...
const EditBooking = () => {
    ...
    const ALL_BOOKING_DATA = JSON.parse(window.localStorage.getItem("all_booking_data"));
    const filteredData = ALL_BOOKING_DATA?.filter(item => item?._id === booking_id);
    const productDetails = filteredData[0]?.product_details;
    ...
    const [inputFields, setInputFields] = useState(productDetails);

    const [formData, setFormData] = useState({
        destination: filteredData[0]?.destination,
        departure_dest: filteredData[0]?.departure_dest,
        shipment_date_time: filteredData[0]?.shipment_date_time,
        flight: filteredData[0]?.flight,
        customer_name: filteredData[0]?.customer_name,
        customer_phone: filteredData[0]?.customer_phone,
        customer_email: filteredData[0]?.customer_email,
        customer_address: filteredData[0]?.customer_address,
        product_details: [],
        _userID: user?.id,
        totalWeight: filteredData[0]?.totalWeight,
        dimension: filteredData[0]?.dimension,
        chargeableWeight: filteredData[0]?.chargeableWeight
    })
    ...
    const handleSubmit = (e) => {
        ...
        // console.log(data);
        dispatch(updateBookingData({ booking_id, data, navigate }));
    };
