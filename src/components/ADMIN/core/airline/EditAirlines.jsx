...
import { editAirlines } from '../../../../services/slices/ADMIN/AirlineSlice';
import Loader from '../../../../util/Loader';

const EditAirlines = () => {
    const { airline_id } = useParams();
    const AIRLINE_DATA = JSON.parse(window.localStorage.getItem("airline_data"));
    const filteredData = AIRLINE_DATA?.filter(item => item?._id === airline_id);

    const [formData, setFormData] = useState({
        airline: filteredData[0]?.airline,
        person_name: filteredData[0]?.person_name,
        person_designation: filteredData[0]?.person_designation,
    ...
    const { loading } = useSelector(state => state.airlineSlice);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const data = { ...formData };
        dispatch(editAirlines({ airline_id, data, navigate }));
    }
                                ...
                                <form onSubmit={handleSubmit}>
                                    <div className="card mb-0">
                                        ...
                                        <div className="card-footer">
                                            <button className="btn btn-sm btn-primary">Update Airline</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
